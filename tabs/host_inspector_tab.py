...
from PyQt6.QtGui import QColor, QBrush, QKeySequence, QShortcut, QCursor
...
class HostScanThread(QThread):
    """
    Runs an nmap scan against a single host with version detection (-sV).
    Emits a list of dicts: {port, state, service, version, protocol}.
    """
    ...
    MODES = {
        ...
        "full":   "-p-",
        "custom": None,   # caller sets custom_ports
    }

    def __init__(self, ip: str, mode: str = "quick",
                 custom_ports: str = "",
                 use_sudo: bool = True,
                 password: str = "",
                 parent=None):
    ...
    def run(self):
        ...
        if self.mode == "custom":
            if not self.custom_ports:
                self.error_occurred.emit("No ports specified for custom scan.")
                return
            port_arg = f"-p {self.custom_ports}"
        else:
            port_arg = self.MODES[self.mode]
        ...
        # Flatten the list since port_arg may contain spaces
        cmd_flat = []
        for part in cmd:
            cmd_flat.extend(part.split() if " " in part else [part])
        ...
        try:
            ...
            if proc.returncode != 0:
                if self.use_sudo and "incorrect password" in stderr.lower():
                    ...
                    self.error_occurred.emit(f"Nmap error: {stderr.strip()}")
                return
...
class HostInspectorTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._sudo_cb = QCheckBox("Use sudo")
        self._sudo_cb.setChecked(True)
        self._sudo_cb.setToolTip(
            "Sudo enables SYN scanning and more accurate version detection.\n"
    ...
    def _start_scan(self):
        ...
        if use_sudo:
            if self.parent_window.cached_password:
                password = self.parent_window.cached_password
            else:
                from tabs.scanner_tab import SudoDialog
                dlg = SudoDialog(self)
                if not dlg.exec():
                    return
                password, remember = dlg.get_data()
                if not password:
                    return
                if remember:
                    self.parent_window.cached_password = password
